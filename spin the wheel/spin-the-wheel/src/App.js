  ...
  const runSpin = (pool, message) =>
    new Promise((resolve) => {
      ...
      spinTimeoutRef.current = window.setTimeout(() => {
        ...
        // The top pointer maps directly to the rotated slice angle.
        // Adding half a slice shifts selection into the neighboring segment.
        const winnerIndex = Math.floor(pointerAngle / anglePerSlice) % pool.length;
        ...
        <aside className="side-card compact-card">
          {mode === 'single' ? (
            ...
            <>
              <div className="card-head compact-head">
                <h2>Groups</h2>
                <p>Use the copy button to copy all generated groups.</p>
              </div>
